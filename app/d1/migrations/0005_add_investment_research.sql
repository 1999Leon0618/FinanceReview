...
CREATE INDEX idx_watchlist_owner_enabled ON watchlist_items(owner_key, is_enabled, updated_at DESC);
CREATE INDEX idx_research_notes_owner_date ON research_notes(owner_key, market_scope, trading_date DESC, note_type);
CREATE INDEX idx_research_sources_note ON research_note_sources(note_id, published_at DESC);
CREATE INDEX idx_research_revisions_note ON research_note_revisions(note_id, revision DESC);
CREATE INDEX idx_research_todos_owner_status ON research_todos(owner_key, status, scheduled_for, created_at DESC);
