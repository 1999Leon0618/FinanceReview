...
週報依序呈現 `Portfolio Snapshot`、本週市場、`Portfolio Attribution`、`Portfolio Risk`、個股重要事件、下週觀察與 `Data Quality`。投資組合歸因只使用相鄰快照已記錄的資金流、市場與匯率變動比例；證據不足時不推測原因。超過單次上限的自選標的、研究報告或待辦會依既有排序截斷，省略筆數會列在 `Data Quality`。
