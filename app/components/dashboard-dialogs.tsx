...
export function SnapshotEditor({
  ...
  const hasDraft =
    ...
    cashFlows.length > 0 ||
    customCapturedAt;
  const closeWithCheck = (action: () => void) => {
    if (busy) return;
    if (
      hasDraft &&
      !window.confirm("尚有未儲存的快照資料，確定要離開並放棄這次修改嗎？")
    )
      return;
    action();
  };
  ...
  return (
    <Modal
      onClose={() => closeWithCheck(onClose)}
      wide
      labelledBy="snapshot-editor-title"
    >
      <div className="snapshot-editor-header">
        ...
        <button
          ...
          aria-label="關閉"
          onClick={() => closeWithCheck(onClose)}
        >
          <X size={18} />
        </button>
      </div>
      <div className="space-y-6 p-7 max-sm:p-4">
            ...
            <div className="snapshot-manual-entry">
              ...
              <div>
                <button
                  type="button"
                  disabled={!!busy}
                  onClick={() => closeWithCheck(onManageAccounts)}
                  className="secondary"
                >
                  管理一般帳戶
                </button>
                <button
                  type="button"
                  disabled={!!busy}
                  onClick={() => closeWithCheck(onManageCreditCards)}
                  className="secondary"
                >
                  管理信用卡
                </button>
        ...
        <div className="sticky bottom-0 z-10 -mx-7 -mb-7 flex flex-wrap justify-end gap-3 border-t border-[#dce4dd] bg-white/95 px-7 py-5 shadow-[0_-12px_30px_rgba(22,45,32,.06)] backdrop-blur-xl max-sm:-mx-4 max-sm:-mb-4 max-sm:px-4">
          ...
          <button className="secondary" onClick={() => closeWithCheck(onClose)}>
            取消
          </button>
          <button
