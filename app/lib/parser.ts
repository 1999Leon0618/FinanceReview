...
以下三種輸入都完全合法，unsupportedReason 必須為 null：
...
function findObject(value: unknown): unknown {
  if (value && typeof value === "object") {
    ...
    if ("accountUpdates" in row && "positionUpdates" in row && "sales" in row)
      return row;
...
const accountName = (value: Record<string, unknown>) =>
  string(
    value.accountName ?? value.account_name ?? value.account ?? value.name,
  );
...
function normalizePatch(value: unknown): unknown {
  ...
  return {
    unsupportedReason:
      source.unsupportedReason ?? source.unsupported_reason ?? null,
    accountUpdates: array(source.accountUpdates ?? source.account_updates).map(
      (item) => {
        ...
        return {
          ...
          accountType: string(
            update.accountType ?? update.account_type,
            "brokerage",
          ).toLowerCase(),
    ...
    positionUpdates: array(
      source.positionUpdates ?? source.position_updates,
    ).map((item) => {
      const update = row(item);
      const type = string(
        update.securityType ?? update.security_type ?? update.type,
        "stock",
      ).toLowerCase();
      return {
        accountName: accountName(update),
        ...
        averageCost: string(
          update.averageCost ?? update.average_cost ?? update.cost,
        ),
      };
    }),
    sales: array(source.sales).map((item) => {
      const sale = row(item);
      return {
        accountName: accountName(sale),
        market: string(sale.market, "TWSE").toUpperCase(),
        symbol: string(sale.symbol ?? sale.code).toUpperCase(),
        soldAt: string(sale.soldAt ?? sale.sold_at ?? sale.date),
        salePrice:
          sale.salePrice === undefined && sale.sale_price === undefined
            ? null
            : string(sale.salePrice ?? sale.sale_price),
...
function mergeDeterministicUpdates(
  ...
  const hasValidUpdate =
    ...
    patch.positionUpdates.length > 0 ||
...
export async function parseNaturalLanguage(
  ...
  const stateDir = await mkdtemp(
    path.join(tmpdir(), "finance-review-openclaw-"),
  );
  ...
  try {
    ...
    const parsed = parserPatchSchema.parse(normalizePatch(object));
    return mergeDeterministicUpdates(rawInput, parsed);
  } catch (error) {
...
export function buildProposal(
  ...
  return {
    ...
    unsupportedReason: patch.unsupportedReason,
  };
}
