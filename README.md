...
- 完整結算持倉全部賣出；一般證券將淨入帳加入指定帳戶，期貨則依行情差額逐次更新權益並保存已實現損益。
...
全部賣出會在同一個資料庫交易內建立一份不含該持倉的新快照，並保存成本基礎及已實現損益。一般證券的淨入帳為成交總額減手續費與交易稅，且會加入指定帳戶；期貨帳戶首次建立快照時填入與持倉行情同時點的券商權益數，之後每次更新行情都會依新舊點數差額自動更新權益，平倉時再依最後行情到成交價的差額更新權益並扣除費稅，從進場均價至成交價的完整已實現損益則另外保存在賣出紀錄。部分賣出仍請在新快照填寫目前剩餘數量與平均成本。
...
更正既有持倉的標的代碼後，系統會同步更新該持倉目前對應的標的，自選清單與之後的賣出紀錄會使用更正後代碼；先前快照仍保留當時記錄的代碼。既有持倉 ID 不能移到另一個帳戶，跨帳戶持倉需在目標帳戶建立新持倉。
...
「設定」頁面的「報告設定」提供繁體中文、英文、日文、AI 模型、投資目標、期限、風險承受度與 API Key 管理。使用者也可開關自動週報並選擇星期、整點／半點時間與時區；production Worker 每 30 分鐘檢查一次到期排程，同一使用者每週最多產生一份排程報告。行情面板不需要 API Key；產生週報時會提示先前往設定頁儲存金鑰，手動按鈕則每按一次另存一份。
...
- 個股事件只保留實際持有的股票，排除 ETF 與基金，依持倉權重由高到低排列；同權重時再比較事件關聯分數，每檔最多保留兩則並取全體前五則。下週事件也會依持倉關聯與事件類別排序後取前五則。送入研究流程的自選清單會排除已出現在持倉配置的標的，避免同一標的重複研究。
- 現金與期貨分析各自提供使用者開關，預設關閉。現金只提供占總資產與各幣別占比；期貨只提供方向、到期月份及名目曝險占淨值比例，且名目價值不會加入資產總額。
...
送至 OpenAI 的資料僅有未與持倉重複的自選標的、公開行情、從最新快照計算的股票／ETF／基金持倉比例，以及使用者填寫的投資背景；使用者開啟選項時，另加入現金配置比例或期貨名目曝險比例。資料不包含結構化的帳戶餘額、持倉數量、期貨口數或金額。投資背景未填齊時仍會整理市場，但不輸出個人化買賣方向。報告記錄產生時的語言、模型、資料期間與使用的證據；AI 建議包含依據、觸發條件與風險，並不會自動下單。OpenAI API 的費用由使用者金鑰所屬帳戶承擔。
