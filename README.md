...
- 完整結算持倉全部賣出；一般證券將淨入帳加入指定帳戶，期貨則保存已實現損益並由下一份快照更新帳戶權益。
- 排除外部投入與提領後估算投資績效、年化報酬與最大回撤，並和臺灣加權指數、SPY 或 VT 比較。
- 啟動時顯示資料更新時效，並檢查總額、行情、匯率、貸款還款與期貨契約月份。
- 透過 JSON 匯出／匯入備份，並可在畫面上暫時隱藏財務數字；日期與最後更新時間可依使用者選擇的語言及時區顯示。
...
5. 要結束持倉時按「全部賣出」，確認成交價、結算帳戶、手續費與交易稅；一般證券會更新現金，期貨會保存已實現損益並等待下一份快照更新券商權益。
...
全部賣出會在同一個資料庫交易內建立一份不含該持倉的新快照，並保存成本基礎及已實現損益。一般證券的淨入帳為成交總額減手續費與交易稅，且會加入指定帳戶；期貨則保存從進場均價至成交價的完整已實現損益與費稅，但不會由平倉操作自動改寫帳戶權益，平倉後請在下一份快照填入券商顯示的最新權益數。部分賣出仍請在新快照填寫目前剩餘數量與平均成本。
...
每頁日期與最後更新時間會依「設定」頁儲存在瀏覽器的語言及時區偏好顯示；旁邊另列目前環境、`package.json` 版本及七碼 Git commit。GitHub Actions 會從 `GITHUB_SHA` 自動帶入部署版本，滑鼠停留可查看建置時間。
